...
from models import HGNN, MVHGNN, MVCHGNN
...
class DEAPTrainer:
    ...
    def _train_model(self, model, xs, label, idx_train, idx_test, G_list):
        ...
        with open(self.log, 'w') as f:
            for epoch in range(self.n_epoch):
                ...
                # 前向传播
                optimizer.zero_grad()
                score = model.forward(feature, G_list)
    ...
    def train_fc(self, task):
        # 生成所需数据和模型
        fts, H_list = self.loader.load_DEAP(ft_in_use=[0, 1, 2, 3], k_list=self.k_list)
    ...
    def train_ensemble(self, task):
        ...
        for k in self.k_list:
            fts_k, H_mat_k = self.loader.load_DEAP(ft_in_use=[0, 1, 2, 3], k_list=[k])
            fts, H_mat = fts + fts_k, H_mat + H_mat_k
        labels = self.loader.label
        n_modal = len(fts)
        label = Tensor(labels[task]).long().to(self.device)
...
def main():
    mytrainer = DEAPTrainer()
    mytrainer.train_ensemble('Valence')
